...
import sampling_3h_2000 as sampling
from sampling_3h_2000 import ReverseDiffusionPredictor,LangevinCorrector,AnnealedLangevinDynamics ,EulerMaruyamaPredictor,AncestralSamplingPredictor
  ...
  for correct in corrects:
    # for check_num in checkpoint_num:
      ...
      if sde.lower() == 'vesde':
        ...
        ckpt_filename_A = '/home/qgl/sanshe/exp_WIDS_binary/checkpoints/checkpoint_10.pth'
        ckpt_filename_3h = '/home/qgl/sanshe/exp_3h_WIDS_binary/checkpoints/checkpoint_10.pth'
      ...
      for j in range(0, 1, 1):
        ...
        ssim_tc_n = []
        for k in range(4,5):
            #img_gt = np.load(f'/home/qgl/sanshe/rec_img/lsun/gt4.npy')
            img_gt = cv2.imread(f'/home/qgl/sanshe/rec_img/cross_data/gt/gt{k}.png', cv2.IMREAD_GRAYSCALE)
            # img_gt = cv2.imread(f'/home/qgl/桌面/size112/gt{k}.png', cv2.IMREAD_GRAYSCALE)

            #tc = np.load(f'/home/qgl/sanshe/rec_img/lsun/bt4.npy')
            tc = cv2.imread(f'/home/qgl/sanshe/rec_img/cross_data/bz/bz{k}.png', cv2.IMREAD_GRAYSCALE)
            # tc = cv2.imread(f'/home/qgl/桌面/size112/bz{k}.png', cv2.IMREAD_GRAYSCALE)

            #rec_data = np.load(f'/home/qgl/sanshe/rec_img/lsun/bt4.npy')
            rec_data = cv2.imread(f'/home/qgl/sanshe/rec_img/cross_data/bt/bz{k}.png', cv2.IMREAD_GRAYSCALE)
            # rec_data = cv2.imread(f'/home/qgl/桌面/size112/bt{k}.png', cv2.IMREAD_GRAYSCALE)
            ...
            for i in range(1):
              ...
              #x, psnr, ssim,psnr_tc,ssim_tc= sampling_fn(model_A,data=img_gt,data_ob=rec_data, tc=tc,k=k)#model_3h
              x, psnr, ssim, psnr_tc,ssim_tc = sampling_fn(model_A,model_3h,img_gt,rec_data, tc,k)#model_3h
              psnr_n.append(psnr)
              ssim_n.append(ssim)
              psnr_tc_n.append(psnr_tc)
              ssim_tc_n.append(ssim_tc)

        # min_1 = x.min()
        # max_1 = x.max()
        # x = (x - min_1) / (max_1 - min_1)  # 归一化
        psnr_result = sum(psnr_n) / (len(psnr_n))
        psnr_tc_result = sum(psnr_tc_n) / (len(psnr_tc_n))
        ssim_result = sum(ssim_n) / (len(ssim_n))
        ssim_tc_result = sum(ssim_tc_n) / (len(ssim_tc_n))
        print('psnr_avg ', psnr_result)
        print('ssim_avg ', ssim_result)
        print('psnr_tc_avg ', psnr_tc_result)
        print('ssim_tc_avg ', ssim_tc_result)
