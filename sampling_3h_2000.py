...
def get_pc_sampler(sde_3h, sde_A, shape,predictor, corrector, inverse_scaler, snr,
  ...
  """
    ...
    def pc_sampler(model_A,model_3h,data,data_ob, tc,k):
                    ...
                    with torch.no_grad():
                        ...
                        # dcll = torch.from_numpy(dcll).cuda()
                        # dclh = torch.from_numpy(dclh).cuda()
                        # dchl = torch.from_numpy(dchl).cuda()
                        # dchh = torch.from_numpy(dchh).cuda()

                        #data_ob_3h = dc[1:, ...]  # 3，128，128
                        ...
                        for i in range(start,end):#按散射来，可调整
                            ...
                            idwt_x[:,:,x:y,x:y] = (idwt_x[:,:,x:y,x:y] - idwt_x[:,:,x:y,x:y].min()) / (idwt_x[:,:,x:y,x:y].max() - idwt_x[:,:,x:y,x:y].min())
                            ...
                            idwt_x[:,:,x:y,x:y] = (idwt_x[:,:,x:y,x:y] - torch.min(idwt_x[:,:,x:y,x:y])) / (torch.max(idwt_x[:,:,x:y,x:y]) - torch.min(idwt_x[:,:,x:y,x:y]))
                            ...
                            psnr = compare_psnr(data_temp[x:y,x:y]* 255, data[x:y,x:y], data_range=255)
                            ssim = compare_ssim(data_temp[x:y,x:y]* 255, data[x:y,x:y],multichannel=True,  data_range=255)

                            # print(data_ob.shape)
                            # data_ob1 =  np.squeeze(data_ob)# 256,256
                            # data_ob1 = data_ob1.cpu().numpy()

                            psnr_tc = compare_psnr(tc1[x:y,x:y] * 255, data[x:y,x:y], data_range=255)
                            ssim_tc = compare_ssim(tc1[x:y,x:y] * 255, data[x:y,x:y], multichannel=True,data_range=255)

                            save_path = "./review_ours_shice"
                            filename = f'image{i}.png'
                            save_path = os.path.join(save_path, filename)
                            Image.fromarray((data_temp[x:y,x:y] * 255).astype(np.uint8)).save(save_path)

                            print(i, 'PSNR:', psnr, 'SSIM:', ssim)

                            images_list.append(data_temp.copy())
                            ...
                            if (ssim > ssim_max) & (ssim != 1):
                                max_1 = i
                                ssim_max = ssim
                                ssim_max_psnr = psnr

                        print("a:", a)
                        print("psnr_max:", max_2, psnr_max)
                        print("psnr_max_ssim:", max_2, psnr_max_ssim)
                        print("ssim_max:", max_1, ssim_max)
                        print("ssim_max_psnr:", max_1, ssim_max_psnr)

                        print("TC psnr：", psnr_tc)
                        print("TC ssim：", ssim_tc)

                        if max_2 != -1:
                            best_image = images_list[end-start-1]
                            save_path = './review_ours'
                            os.makedirs(save_path, exist_ok=True)
                            filename = f'rec_{k}_{psnr}_{ssim}.png'
                            save_path = os.path.join(save_path, filename)
                            Image.fromarray((best_image[x:y,x:y] * 255).astype(np.uint8)).save(save_path)

                        return data_temp[x:y,x:y] * 255, psnr, ssim,psnr_tc,ssim_tc
    return pc_sampler
