...
class GetCT(Dataset):
    ...
    def __getitem__(self,index):
        ...
        ori_data = img[None, None, ...]


        ori_data = np.float32(ori_data)

        ori_data = self.img_normalized(ori_data)
        ...
        #在这里的ori_data加mask
        #ori_data = mask*ori_data
        
        dwt_img = dwt_data(ori_data)#返回小波后图片
        #print(sinogram.shape)

        #return sinogram[None,...]
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetCT(root= "./train_img_fation")
  test_dataset = GetCT(root= "./train_img_fation")
